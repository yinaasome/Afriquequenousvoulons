...
def init_database():
    ...
    try:
        ...
        # Vérifier si la base est déjà initialisée avec un flag
        if hasattr(init_database, '_already_executed'):
            print("⚠️ Base déjà initialisée (skip)")
            return True
        init_database._already_executed = True
        
        # OU vérifier avec une collection de métadonnées
        if "system_initialized" in db.list_collection_names():
            if db.system_initialized.find_one({"init": True}):
